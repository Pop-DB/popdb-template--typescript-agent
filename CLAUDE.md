...
This is an **agent-mode** app: a single admin user, no frontend, no public signup. Event handlers are the core — they run server-side TypeScript in isolated Docker containers, processing events and calling integrations.

## Project Structure

```
popdb.config.ts          # Event types + handler registration
handlers/                # Handler files (one per handler)
...
// decision = { event: 'lead.new', payload: { company: 'Acme', interest: 'enterprise plan', priority: 'high' }, metadata: { model, candidates } }
...
For agent-mode apps, tables should default to `access: "internal"` — accessible to event handlers (service role) and the admin user, but not to anonymous users. No `user_id` column needed.
...
4. **Start the API**: `start_postgrest` (if handlers use `ctx.db`)
